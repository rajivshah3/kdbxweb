...
var crypto = global.process && global.process.versions && global.process.versions.node ? require('crypto') : null;
*/

var rng = require('react-native-securerandom');
...
function hmacSha256(key, data) {
    if (subtle) {
        ...
        return new Promise(function(resolve) {
            var hmac = crypto.createHmac('sha256', Buffer.from(key));
...
AesCbcNode.prototype.encrypt = function(data, iv) {
    ...
    return Promise.resolve().then(function() {
        var cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(that.key), Buffer.from(iv));
...
AesCbcNode.prototype.decrypt = function(data, iv) {
    var that = this;
    return Promise.resolve().then(function() {
        var cipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(that.key), Buffer.from(iv));
        var block = cipher.update(Buffer.from(data));
        return ByteUtils.arrayToBuffer(Buffer.concat([block, cipher.final()]));
    }).catch(function() { throw new KdbxError(Consts.ErrorCodes.InvalidKey, 'invalid key'); });
};
...
function safeRandom(len) {
    ...
    while (len > 0) {
        ...
        rng.generateSecureRandom(segmentSize).then(function (bytes) {
          randomBytesSegment = bytes;
        });
        len -= segmentSize;
